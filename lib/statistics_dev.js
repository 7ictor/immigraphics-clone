...
function globalFetcher () {
    for (var eins = 0; eins < places.length; eins++) {
        ...
        paises[eins] = {
            'nombre': places[eins],
            'genero': [
                {'name': 'male', 'value': tmpM},
                {'name': 'female', 'value': tmpF}
            ]
        };
    }
...
// The db is to slow to fetch all the results one by one! Site gets stuck!!!
// Use local?
...
// Graphics methods ----------- This section has methods which will be loaded on demand (when a button is clicked)
...
// The dropdown (only one) for general graphics should include an option *Countries* and an option *Death Causes* 
// Activates general graphics
function generals() {
        doubleBarGraph('tablita.csv', 'Deaths in various corridors across the border.');
}        

// The dropdowns here should include two to change the time span of the graph and one to change *Countries* or *Death Causes*. After the selection of those two, a 4th dropdown menu should appear to show which countries or which death causes are plottable.
